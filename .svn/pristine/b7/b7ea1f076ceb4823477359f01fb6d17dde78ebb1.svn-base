...
public class Manager_Controller_4W extends BaseMultiController {
	...
	public void add_manager(Manager manager) {
		...
		//由于身份表与管理员表的字段相同，插入的时候识别不出来，所以在这进行插入
		t_s_user_manager_role.setName(manager.getUser_manager_role_name());
		try {
			...
			manager.setUser_manager_role_id(t_s_user_manager_role.getId());
			t_u_manager_Service_Iface.insert_t_u_manager(manager);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void delete_all_manager(T_u_manager t_u_manager, T_s_user_manager_role t_s_user_manager_role) {
		try {
			...
			manager_Service_Iface.delete_all_user_manager_role(t_s_user_manager_role);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
